...
namespace VAshe
{
    class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Game.PrintChat("VAshe by ViHuRa loaded!");
        }
        ...
        {
            bool useW = W.IsReady() && menu.SubMenu("wave").Item("UseW").GetValue<bool>();
            Obj_AI_Hero targetW = SimpleTs.GetTarget(W.Range, SimpleTs.DamageType.Physical);
            if (useW)
            {
                W.CastIfHitchanceEquals(targetW, HitChance.Medium);
        ...
        {
            if ((!menu.SubMenu("combo").Item("UseQ").GetValue<bool>()) || (!menu.SubMenu("misc").Item("UseQ").GetValue<bool>())) return;
        ...
        {
            // Initialize the menu
            menu = new Menu("VAshe", champName, true);
            ...
            // WaveClear
            Menu wave = new Menu("Wave", "wave");
            menu.AddSubMenu(wave);
            wave.AddItem(new MenuItem("UseW", "Use W harras").SetValue(true));
            ...
            misc.AddItem(new MenuItem("UseQ", "Block Q on minions").SetValue(true));
